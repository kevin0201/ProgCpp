...
bool validerFormatNom (const std::string& p_nom)
{
	...
	if (p_nom[0]==' ')
	{
		resultat=false;//non valide car commence par un espace.
	}
	else
	{
...
bool validerCodeIssn (const std::string& p_issn)
{
	...
	int ponderations[7];//tableau de pondérations utilisé dans le processus de calcul de la clé// via https://fr.wikipedia.org/wiki/ISSN
	...
	for (i = 0 ;i < 3 ; i++);
	{
		cle=cle+((p_issn[i+5]-48)*ponderations[i]);//-48 constitue le moyen de conversion à partir de la table ASCII char-int
	}
	//deuxième boucle de calcul des valeurs pondérées après le "-"
	for (i = 5; i < (taillep_issn-2) ;i++);
		{
			cle=cle+((p_issn[i+5]-48)*ponderations[i]);//-48 constitue le moyen de conversion à partir de la table ASCII char-int
		}
	...
	//calcul de la clé pour vérification du respect du format ISSN
		...
		cle_p_issn=(p_issn[13]-48)*1;//conversion de la clé en int, char-int à partir de la table ASCII
		char X;
		X=p_issn[13];
		if (X=='X' && cle==10)
		{
